...
def test_dynamic_res_concurrent_res_increment(ray_start_cluster):
    ...
    def wait_func(id_str):
        ray.get(ray.ObjectID(id_str)) # To make the task wait till signalled by driver
    ...
    task = wait_func._remote(args=[WAIT_OBJECT_ID_STR], resources={res_name: 1})
...
def test_dynamic_res_concurrent_res_decrement(ray_start_cluster):
    ...
    task = wait_func._remote(args=[WAIT_OBJECT_ID_STR], resources={res_name: 4})
    time.sleep(0.2) # Sleep to make sure the wait_func is launched before updating resource

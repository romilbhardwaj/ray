...
# class SoftConstraintManager(object):
#     def __init__(self):
#         self.placement_successful = {}  # Map from task id to bool
#         self.task_id_counter = 0
#
#     def launch_callback(self, task_id):
#         if self.placement_successful[task_id]:
#             print("[LaunchCallback] Task {} already launched, returning false.".format(task_id))
#             return False
#         else:
#             print("[LaunchCallback] Task {} launched successfully. Marking as launched.".format(task_id))
#             self.placement_successful[task_id] = True
#             return True
#
#     def launch_task(self, ord_soft_constraints, hard_constraints, func, args):
...
#         self.task_id_counter+=1
#         self.placement_successful[this_task_id] = False
#
#         @ray.remote
#         def func_wrapper(manager, task_id, func_args):
#             is_valid_run = manager.launch_callback(task_id)
#             if is_valid_run:
#                 return func(*func_args)
#             return None
#
#         for soft_constraint_instance in ord_soft_constraints:
#             print("[LaunchTask] Trying soft constraint {}".format(soft_constraint_instance))
#             combined_constraint = {**soft_constraint_instance, **hard_constraints}  # Hard will override any conflicting soft constraints
#             attempt = func_wrapper._remote(args=[self, this_task_id, args], resources=combined_constraint)
#
#             # Wait for a few seconds for launch_callback to be called and self.placement_successful to be updated.
#             time.sleep(2)
#             if self.placement_successful[this_task_id]:
#                 print("[LaunchTask] Soft constraint {} satisfied. Task launched.".format(soft_constraint_instance))
#                 return attempt
#
#         print("[LaunchTask] None of the soft-constraint was satisfied. Launching with only hard constraint and returning.")
#         attempt = func_wrapper._remote(args=[self, this_task_id, args], resources=hard_constraints)
#         return attempt
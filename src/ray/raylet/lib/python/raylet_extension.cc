...
static PyObject *PyRayletClient_CreateResource(PyRayletClient *self, PyObject *args) {
  const char *resource_name;
  int resource_name_length;
  double capacity;
  UniqueID client_id;
  if (!PyArg_ParseTuple(args, "s#dO&", &resource_name, &resource_name_length, &capacity, PyStringToUniqueID, &client_id)) {
    return NULL;
  }
  std::string resource_name_str(resource_name);
  auto status = self->raylet_client->CreateResource(resource_name_str, capacity, client_id);
  RAY_CHECK_OK_PREPEND(status, "[RayletClient] Failed to create resource.");
  Py_RETURN_NONE;
}

static PyObject *PyRayletClient_DeleteResource(PyRayletClient *self, PyObject *args) {
    const char *resource_name;
    int resource_name_length;
    UniqueID client_id;
    if (!PyArg_ParseTuple(args, "s#O&", &resource_name, &resource_name_length, PyStringToUniqueID, &client_id)) {
        return NULL;
    }
    std::string resource_name_str(resource_name);
    auto status = self->raylet_client->DeleteResource(resource_name_str, client_id);
    RAY_CHECK_OK_PREPEND(status, "[RayletClient] Failed to delete resource.");
    Py_RETURN_NONE;
}
...
static PyMethodDef PyRayletClient_methods[] = {
    ...
    {"create_resource", (PyCFunction)PyRayletClient_CreateResource, METH_VARARGS,
    "Create resource with the specified name and capacity."},
    {"delete_resource", (PyCFunction)PyRayletClient_DeleteResource, METH_VARARGS,
     "Delete a resource with the specified name."},
    {NULL} /* Sentinel */
};

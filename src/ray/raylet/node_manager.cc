...
void NodeManager::ResourceCreateUpdated(const ClientTableDataT &client_data) {
  ...
  for (const auto &resource_pair : difference_set.GetResourceMap()) {
    ...
    if (client_id == local_client_id){
      local_available_resources_.AddOrUpdateResource(resource_label, new_resource_capacity);
    }
  }
...
void NodeManager::ProcessClientMessage(
  ...
  case protocol::MessageType::DeleteResourceRequest: {
      ProcessDeleteResourceRequest(client, message_data);
  } break;
...
void NodeManager::ProcessDisconnectClientMessage(
  ...
  if (is_worker) {
    ...
    local_available_resources_.ReleaseConstrained(task_resources, cluster_resource_map_[client_id].GetTotalResources());
    ...
    local_available_resources_.ReleaseConstrained(lifetime_resources, cluster_resource_map_[client_id].GetTotalResources());
...
void NodeManager::ProcessCreateResourceRequest(const std::shared_ptr<LocalClientConnection> &client, const uint8_t *message_data) {
  ...
  RAY_CHECK_OK(gcs_client_->client_table().Append(JobID::nil(), client_table.client_log_key_, data_shared_ptr, nullptr));
}

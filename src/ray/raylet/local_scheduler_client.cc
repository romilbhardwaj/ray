...
void local_scheduler_create_resource(LocalSchedulerConnection *conn,
                                     const std::string &resource_name,
                                     const int capacity,
                                     const ray::ClientID &client_Id) {
  flatbuffers::FlatBufferBuilder fbb;
  auto message = ray::protocol::CreateCreateResourceRequest(
          fbb, fbb.CreateString(resource_name), capacity, to_flatbuf(fbb, client_Id));
  fbb.Finish(message);

  write_message(conn->conn,
          static_cast<int64_t>(ray::protocol::MessageType::CreateResourceRequest),
          fbb.GetSize(),fbb.GetBufferPointer(), &conn->write_mutex);
}